...
if (isset($_POST['id'])) {
    ...
    $stmt = $GLOBALS['conn']->prepare("SELECT sitio_turistico.id_sitio, sitio_turistico.nombre, 
    sitio_turistico.latitud, sitio_turistico.longitud, sitio_turistico.descripcion, 
    sitio_turistico.horarios, sitio_turistico.tarifa as arancelado, localidad.nombre as nombre_localidad FROM sitio_turistico JOIN localidad on sitio_turistico.id_localidad = localidad.id_localidad WHERE id_sitio = :id ");
    ...
    $stmt = $GLOBALS['conn']->prepare("SELECT id_sitio, nombre, latitud, longitud, descripcion FROM sitio_turistico WHERE estado = 1");
}
